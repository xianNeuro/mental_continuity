#!/usr/bin/env python3
"""
derive_carver_PMC_story-phase_inter-subj.py

Derives ``data/derived/carver_PMC_story-phase-mean_skip0-use10_inter-subj.xlsx``
from the PMC MVP files only.

The "story-phase" inter-subject pattern correlation is the pre×pre quadrant
of the inter-subject TTC matrix around each interruption onset — i.e. the
mean of the 10×10 sub-block whose rows and columns both correspond to the
ten repetition times immediately before interruption onset (skip 0, use 10).
This quantity is named "story-phase" PMC pattern persistence throughout the
outputs.
...
Output xlsx columns:
  task, condition, roi, subject_id, story-phase-mean_overall.
